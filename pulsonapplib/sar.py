...
def generate_range_vector(x,y,platform, speed):
    res = []
    for p in range(0, speed):
        res.append(get_pixel_range(x,y,platform[p * round(len(platform) / speed)]))
    return res
...
def integrate_pixel_intensity(ranges,pulses, x, y, speed):
    pixel_intensity = 0
...
def parse_args(args):
    ...
    parser.add_argument('-m', '--mode', dest='mode', type=str, default='normal', help='Mode in which to run the program', choices=('fourier', 'quickf', 'quicki', 'normal'))
...
def fourierShift(file, size, pixels, mode, freq, num, speed):
    ...
    for n in range(0, len(platform_positions)):
        spacing = round(len(platform_positions) / speed)
        calc_range = range(0, len(platform_positions), spacing)
        ...
        if n in calc_range:
            ...
            ang_freq = 2 * np.pi * np.arange(-len(range_bins[0]) / 2, len(range_bins[0]) / 2) / (len(range_bins[0]) * drange_time)
            time_distance = 2 * range_array / SPEED_OF_LIGHT

            pulsesofint = np.transpose(np.atleast_2d(pulses[n])) * np.exp(-1j * 2 * np.pi * freq * (range_time - time_distance))
...
def main(file, size, pixels, mode):
    ...
    if mode == 'fourier':
        ...
        for i in range(0, len(sar_image)):
            sar_image[i] = np.abs(fourierShift(file, size, pixels, mode, freq, i, len(platform_positions)))
            if i == len(sar_image) - 1:
                ...
                print(str(perDone) + "%")
    """
    # Quick running for interpolation mode
    if mode is True:
        ProgressValues.is_running = True
        ProgressValues.total_length = IMAGE_RESOLUTION[0]
        sar_image = np.zeros(IMAGE_RESOLUTION)
        perDone = 0
        for i in range(0, len(sar_image)):
            ProgressValues.iteration = i
            sar_image[i] = np.abs(fourierShift(file, size, pixels, mode, freq, i, 50))
            if i == len(sar_image) - 1:
                # ProgressValues.toolbar_percent = 100
                ProgressValues.is_running = False
                print("Done")
            elif math.floor(100 * i / IMAGE_RESOLUTION[0]) > perDone:
                perDone = math.floor(100 * i / IMAGE_RESOLUTION[0])
                # ProgressValues.toolbar_percent = perDone
                ProgressValues.is_running = True
                # print(ProgressValues.toolbar_percent)
                #print(str(perDone) + "%")
    # Quick running for interpolation mode
    elif mode == 'quicki':
        ProgressValues.is_running = True
        ...
        for i in range(len(sar_image)):
            ...
            for j in range(len(sar_image[i])):
                sar_image[len(sar_image)-i-1][len(sar_image[i])-j-1] = np.real(integrate_pixel_intensity( generate_range_vector(j,i,platform_positions, 70), pulses, i, j, 70))
            if i == len(sar_image) - 1:
                # ProgressValues.toolbar_percent = 100
                ProgressValues.is_running = False
                print("Done")
    ...
    # Defines the matrix that will be used the generate the image and fills it in with the intensities
        ...
        for i in range(len(sar_image)):
            ProgressValues.iteration = i
            for j in range(len(sar_image[i])):
                sar_image[len(sar_image)-i-1][len(sar_image[i])-j-1] = integrate_pixel_intensity( generate_range_vector(j,i,platform_positions, len(platform_positions)), pulses, i, j, len(platform_positions))
            if i == len(sar_image) - 1:
                # ProgressValues.toolbar_percent = 100
                ProgressValues.is_running = False
                print("Done")
    ...
    """    
    plt.imshow(np.abs(sar_image)) 
    plt.title('SAR Image', color='#ffcccc')
    plt.show()
