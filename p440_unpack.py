...
def parse_args(args):
    ...
    parser.add_argument('-c --continuous', action='store_true',
            dest='continuous',
			help='Will continuously update plot')
    parser.add_argument('-r --remote', action='store_true',
            dest='remote',
			help='Get plot data remotely via ssh (requires sshpass)')
...
def main(args):
    ...
    if args.visualize and args.continuous:

        if args.remote:
            print("beginning ssh connection")
            os.system("sshpass -p TimHoward ssh pi@192.168.2.1")
            os.system("cd PulsON_Code")
            print("ssh connected")
        ...
        while 1:
            ...
            file_dat = os.popen("cat -v " + args.file).read()
            print(len(file_dat))
            with open('data_buffer', 'wb') as o:
                pickle.dump(file_dat, o)
                #o.write(file_dat)
                o.close()
            data = unpack('./data_buffer')
